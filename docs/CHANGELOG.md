...
- 完善本地 Matroska/MKV 客户端 fallback 的有界解封装和文本字幕能力，支持可定位索引、字幕 cue 以及 `S_TEXT/UTF8`、ASS 和 SSA 的基础样式。
- 增加远程 `.strm` 原生字幕轨道发现与选择，字幕状态绑定当前 `<video>` 生命周期，不改变媒体 URL、播放会话或播放计划。
- 恢复远程 HTTP(S) `.strm` 的 Matroska/WebM 内嵌文本字幕手动选择：浏览器未暴露原生轨道时，选择 SRT/ASS/SSA 后才由浏览器直接对 `externalUrl` 发起有限 CORS/Range 读取，启动客户端音视频与字幕处理，不经过 Lux Relay、不预先抽取、落盘或生成外挂文件。
...
- 修复远程 Matroska 字幕管线自动接管播放导致原生视频受 CORS、Range、索引或 MSE 条件影响的问题；未手动选择字幕时保持原生 `<video>` Direct Play，显式字幕处理失败时撤销字幕并恢复原生播放。
- 修复首个 Range 仅包含有限长度 Segment 的部分内容时被误判为 EBML 越界的问题；索引锚点检查现在允许读取前缀，完整索引解析仍保留严格边界校验。
- 修复远程 Matroska SRT 轨道发现后切换至原生字幕模式、导致显式字幕管线退出的问题；运行时 Matroska 文本轨统一使用 Lux 字幕覆盖层，并补充 WebM 容器识别。
- 修复远程字幕引擎切换后播放位置或播放/暂停状态丢失的问题；交接时按当前媒体源与播放生命周期保存并恢复状态。
- 修复插件同步或旧计划更新时可能覆盖自定义执行计划的问题；现在保留媒体库的自定义计划归属，并在配置镜像与新计划之间同步启停状态。

### Changed

- 管理台注册任务改为“任务类型—执行计划—媒体库运行任务”层级展示，同时保留旧的单库计划 API 和配置镜像以兼容已有客户端。
- 计划触发仍按媒体库创建独立运行任务，沿用扫描锁、去重和资源边界；实时增量扫描优先，全量扫描默认串行。
- 远程 `.strm` 直接使用 `externalUrl`，原生播放优先；浏览器原生能力不足时，客户端 HEVC/MKV 的 Worker、WASM/WebCodecs 和 MSE fallback 仍从 `externalUrl` 读取，不使用 Lux Relay、Lux Direct 或服务端 HLS。
- 显式远程字幕模式也直接从 `externalUrl` 发起有限 CORS/Range 请求；完整 Cues 跳转、上游跨域配置及跨浏览器兼容性阶段门尚待完成。
...
- URL 型和路径型 `.strm` 的 Emby 条目 `Path`、媒体源统一保留原始目标并提供标准代理入口；Lux 仍保留 URL 型 `.strm` 直接访问时的受保护入口和 307 重定向回退。
...
- 支持 NextEmby 发送的 Base64 文本头像上传，并在校验后保存为图片二进制。
...
- 修复服务启动时人物索引恢复无条件重写全部人物关系快照的问题；现在仅在索引状态缺失或确有需要时恢复，并清理失效关系。
- 修复相同标题但来源版本不同的电影被扫描成多个媒体条目的问题；现在合并共享标题并保留各自版本信息。
