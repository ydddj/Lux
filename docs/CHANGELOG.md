...
- 修复 URL/路径型 `.strm` 在 Emby `PlaybackInfo` 中被 Lux 签名直连地址或标准媒体流入口替换、导致外部媒体代理无法提取映射信息的问题；现在保留原始 `DirectStreamUrl` 并关闭 `AddApiKeyToDirectStreamUrl`。
...
- 调整 `.strm` 的 Emby `PlaybackInfo` 兼容策略：URL/路径型目标保留原始 `DirectStreamUrl`，协议解析器目标继续使用 Lux 受保护入口；Web 播放计划不变。
