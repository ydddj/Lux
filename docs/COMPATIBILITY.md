...
兼容 Emby 标准 `DeviceProfile` 时，Lux 会用 `DirectPlayProfiles` 匹配媒体源的容器/音视频编码；当源媒体信息已知且
确认直放 profile 不匹配，或 `MaxStreamingBitrate`（顶层或 `DeviceProfile` 内）低于已知源码率，且客户端允许转码并存在
HLS `TranscodingProfiles` 时，才选择服务端转码。容器/codec/码率信息缺失或仍待探测时按“未知”处理，不会仅因元数据不足而
触发转码；顶层布尔值全部省略时也按此规则协商，并按 Emby 约定将省略的开关视为启用。存在 HLS 转码 profile 的本地 source 会公开 `SupportsTranscoding=true`；实际返回
...
| Hills | 1.8.0 | Android 17，23127PN0CC，arm64-v8a；宿主 macOS arm64 | 未单独验证（已有连接） | 未单独验证（已有会话） | 通过 | 通过；NextEmby `.strm` 代理场景需使用本次第三方播放器 token URL 兼容修复 | 未单独验证 | 未测试 | 2026-08-28 使用独立资源《7天》进入 Hills MPV 播放器并稳定出画面；2026-09-05 HAR 复现 Hills 丢失独立媒体请求鉴权导致 NextEmby `User: unknown`，Lux 已在 URL/路径型 `.strm` 签名播放 URL 写入 `api_key`；需部署后复测 |
...
| 远程 Matroska SRT/ASS/SSA 无 native track | 自动化通过 | 字幕项仍可主动选择；选择后使用签名 `rangeUrl` 进入客户端 Matroska 管线，不创建 `/subtitles/...` 或外部 CDN JavaScript 请求 |
...
- 2026-08-28 通用 Emby 无头播放兼容：发现 HILLS 等 Android 播放器也可能在独立媒体请求中丢失 Emby 鉴权头。`PlaybackInfo` 现在对所有可播放媒体源生成绑定用户、条目和媒体源的短期 HMAC `DirectStreamUrl`；URL/路径型 `.strm` 额外携带当前标准 Emby token 的 `api_key` 并将 `AddApiKeyToDirectStreamUrl` 设为 `true`，本地文件和 SMB/FTP 解析源不把长期 token 写入 URL。标准 `X-Emby-Token`、`X-Emby-Authorization` 和 `api_key` 仍继续接受。`.strm` 播放入口继续使用媒体请求中的播放器 User-Agent 解析有限重定向；自动化回归已覆盖无头本地播放、远程 307 和协议解析器路径。第三方播放器需在远端部署后重新复测。
...
- 2026-09-05 Hills 1.8.0 Android HAR 兼容修复：Hills 在 `Sessions/Playing`、`Progress` 和 `Stopped` 请求中发送 JSON body，但错误声明 `Content-Type: text/plain`，导致播放回调返回 415。Lux 现在仅对这三个 Emby 播放回调按 JSON body 兼容解析，非 JSON body 仍返回 400；远端部署和 Hills 真机复测待完成。
- 2026-09-05 第三方播放器/NextEmby 播放身份兼容修复：Hills 1.8.0 的独立媒体请求只携带 Lux 短期 `luxPlayback...` 票据，且实测忽略 `AddApiKeyToDirectStreamUrl`，NextEmby 无法用自定义参数识别播放用户，因而在回源前记录 `User: unknown` 并拦截。Lux 现在对所有 URL/路径型 `.strm` 播放器在同一签名 URL 中补充标准 `UserId` 身份关联提示和本次标准 Emby token 的 `api_key`，并将 `AddApiKeyToDirectStreamUrl` 设为 `true`；该 `UserId` 提示使用 Lux 登录用户名，以兼容将无头媒体请求映射到本地用户名的代理，Lux UUID 仍只用于 `luxPlaybackUserId` HMAC 绑定。真正授权仍由 Lux 绑定条目、媒体源和用户的 HMAC 票据完成，不开放匿名媒体访问。本地文件和 SMB/FTP 解析源仍不携带长期 token。远端部署后需用 Hills 及其他目标播放器重新验证独立媒体请求。
...
- 2026-09-10 分集 FANART 图片兼容修复：分集剧照历史上使用 `*-thumb` 旁车名以兼容 Emby，但不得与 FFmpeg 的 THUMB 索引共享文件所有权。新 FANART 写回统一使用 `*-fanart`，FFmpeg 新生成的缩略图使用 `*-thumbnail.jpg`，历史 `*-thumb` 仍可读取。启动迁移会将已有分集 FANART 的稀疏索引压缩为从 0 开始，启动修复会为已有的 FANART/THUMB 同路径记录创建独立 THUMB 副本并更新索引，不删除原图。`tests/image_writer.rs`、`tests/metadata_selection.rs` 和 `tests/thumbnails.rs` 已覆盖路径保护、元数据选择、历史冲突修复及迁移回归；这属于服务端图片存储行为，部署后需重新打开受影响剧集确认客户端缓存已刷新。
